...
- **Email protection**: When Exchange Online with the new Office 365 Message Encryption with new capabilities is used to protect and consume email messages, authentication can also use federation with a social identity provider or by using a one-time passcode. Then, the flow is very similar, except that it happens service-side in a web browser session over a temporarily cached copy of the outbound email.

...
|Send a protected email (with attachments that are automatically protected) to users when no trust relationship exists by using federation with social providers or a web browser for viewing.|Does not support sending protecting email when no trust relationship exists.|

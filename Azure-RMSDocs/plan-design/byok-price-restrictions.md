...
There is one exception: **If Exchange Online is enabled but does not support the AzureRMSLicensingEnabled parameter**, you cannot currently use BYOK. Your tenant must be upgraded to support this parameter. Customers who haven't yet enabled Exchange Online for Azure Rights Management will automatically have a tenant that supports this parameter and can therefore use BYOK without restrictions.

...
Word, PowerPoint, Excel, and PDF documents support this SharePoint IRM protection. By default, the protection is restricted to the person who downloads the document. You can change this default with a configuration option named **Allow group protection**, which extends the protection to a group that you specify. For example, if a single group has permission to edit documents in the library, you could specify this group so that the same users can edit the document outside SharePoint, but it's still protected from other users. Or, you might specify a group that isn't granted permissions in SharePoint but need to collaborate on the document outside SharePoint.

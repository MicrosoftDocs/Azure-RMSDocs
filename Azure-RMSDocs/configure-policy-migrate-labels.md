...
|["Just for me" protection](configure-policy-protection.md#example-6-label-that-applies-just-for-me-protection) |Yes|Unlike the Azure portal, the admin centers do not let you save encryption settings that you apply now, without specifying any users. Instead, create a label that applies encryption and specify a user or group with any permissions, that you then change by using the [New-AipServiceRightsDefinition](https://docs.microsoft.com/powershell/module/aipservice/new-aipservicerightsdefinition) cmdlet.|

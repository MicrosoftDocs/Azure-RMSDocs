...
|**Files attached to emails** |Due to a limitation in recent Windows updates, when [Microsoft Outlook is protected by Azure Rights Management](office-apps-services-support.md), scanning Outlook messages may cause any files attached to those messages to be locked after the files are opened. <br><br>To unlock the files, stop the scanner service. Starting the scanner service again does not lock the files again, until the next time the messages are scanned and the files opened.  |

...
|**Content markings in Word**    | Azure Information Protection content [markings](configure-policy-markings.md) are affected in Microsoft headers and footers when the header or footer also contains a table, as follows: </br></br>- **If the header contains a table**, the content marking placement may be incorrect or offset in the header. </br>- **If the footer contains a table**, the content marking may be hidden. </br></br>For more information, see [When visual markings are applied](configure-policy-markings.md#when-visual-markings-are-applied). |

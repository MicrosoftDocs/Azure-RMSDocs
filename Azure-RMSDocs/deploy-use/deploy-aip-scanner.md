...
|If all files in one or more data repositories must have a label<br /><br />- A default label configured as a policy setting|For more information about how to configure the default label setting, see [How to configure the policy settings for Azure Information Protection](configure-policy-settings.md).<br /><br />This default label setting must be in the same policy as the labels in the preceding row. However, this default label setting can be overriden by a different default label that you configure for a data repository.|

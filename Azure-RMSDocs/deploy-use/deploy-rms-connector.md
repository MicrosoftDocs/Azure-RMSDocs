...
Use this information and instructions to deploy the Azure Rights Management (RMS) connector for Azure Information Protection. This connector provides data protection for existing on-premises deployments that use Microsoft Exchange Server, SharePoint Server, or file servers that run Windows Server and File Classification Infrastructure (FCI).
...
The Microsoft Rights Management (RMS) connector lets you quickly enable existing on-premises servers to use their Information Rights Management (IRM) functionality with the Azure Rights Management service from Azure Information Protection. With this functionality, IT and users can easily protect Office documents, pictures, and emails both inside your organization and outside, without having to install additional infrastructure or establish trust relationships with other organizations. 

You can use this connector even if some of your users are connecting to online services, in a hybrid scenario. For example, some users' mailboxes use Exchange Online and some users' mailboxes use Exchange Server. After you install the RMS connector, all users can protect and consume emails and attachments by using the Azure Rights Management connector, and information protection works seamlessly between the two deployment configurations.
...
If you manage your own tenant key for Azure Information Protection (the bring your own key, or BYOK scenario), the RMS connector and the on-premises servers that use it do not access the hardware security module (HSM) that contains your tenant key. This is because all cryptographic operations that use the tenant key are performed in the Azure Rights Management service, and not on-premises.

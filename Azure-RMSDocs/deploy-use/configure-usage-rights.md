...
Alternatively, you can change this encryption inheritance of documents for recipients who view the document in their browser. Consider using this configuration when you don't need to retain the original protection for the document after the user is authenticated. To make this change, use the Exchange Online PowerShell command: `Set-IRMConfiguration -DecryptAttachmentFromPortal $true`. Then, when these recipients download the document, the protection is removed. For more information, see the Office blog post, [Admin control for attachments now available in Office 365 Message Encryption](https://techcommunity.microsoft.com/t5/Security-Privacy-and-Compliance/Admin-control-for-attachments-now-available-in-Office-365/ba-p/204007).       

...
In addition, if [Office 2010](known-issues.md#aip-for-windows-and-office-versions-in-extended-support) is installed locally on an Exchange server, the client app's IRM features might work from that computer after the server is configured to use the connector, but this is not supported.

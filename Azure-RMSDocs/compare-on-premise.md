...
<! -- leave or update? -->| **Client support**|Supports the AIP unified labeling client.<!--need vihita on this one for both protection and consumption activities.--> |Supports the AIP unified labeling client for consumption only, and requires that you install the [Active Directory Rights Management Services Mobile Device Extension](./active-directory-rights-manage-mobile-device.md). |

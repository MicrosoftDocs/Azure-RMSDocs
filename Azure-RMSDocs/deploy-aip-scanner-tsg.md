...
|---------|---------|
|**Authentication errors**     |  - [Authentication token not accepted](#authentication-token-not-accepted) <br>  - [NoAuthTokenException: Client application failed to provide authentication token for HTTP request](#noauthtokenexception-client-application-failed-to-provide-authentication-token-for-http-request)|
|**Policy errors**     | - [Scanner failed to download the DLP policy](#scanner-failed-to-download-the-dlp-policy) <br> - [Policy doesn't include any automatic labeling condition](#policy-doesnt-include-any-automatic-labeling-condition)      |
|**Schema errors**     |   - [Error for a mismatched schema](#error-for-a-mismatched-schema) <br>    - [Database schema is not up to date](#database-schema-is-not-up-to-date)  |
|**Other errors**     |  - [Error occurred while sending the request](#error-occurred-while-sending-the-request) <br>- [Missing content scan job or profile](#missing-content-scan-job-or-profile) <br>- [No repositories configured](#no-repositories-configured) <br>- [No cluster found](#no-cluster-found)   |
|     |         |
...
- In the Azure portal, on the **Nodes** page: `Policy is missing`, or `DLP policy is missing`
...
- In the Azure portal, on the **Nodes** page: `DB schema is not up to date. Run Update-AIPScanner command to update the DB schema`
...
In the Azure portal: **Error: DB schema is not up to date**

**Description:**

This error may occur if you've upgraded your scanner installation but the database schema was not updated in parallel.
...
For example, the following error may occur in the Azure portal on the **Nodes** page:
...
Check your content scan job settings and add at least one repository. For more information, see [Create a content scan job](deploy-aip-scanner-configure-install.md#create-a-content-scan-job).

...
- Use PowerShell to rename the template or delete it.
    ...
    Before you do these actions, consider whether other admins or services are using the template, or have used it in the past. You can identify the template by its GUID that doesn't change, or its name (which can be changed). As a best practice, delete a template only if you are sure that users or services do not have to open documents or emails that were protected by the template.

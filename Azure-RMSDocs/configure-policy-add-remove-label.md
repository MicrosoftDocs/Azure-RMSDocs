...
To add a sublabel, its parent label must be in the same policy, or in the global policy. When you add a sublabel, settings from the main label are not inherited. In this scenario, the main label is supported as a display container for the name and color, and other configuration settings in this main label are not supported for visual markings, protection, and conditions.

...
In addition to the current release, there is now a **preview** version of the Azure Information Protection client that you can only download from the Microsoft Download Center. It offers the same functionality as the Rights Management sharing application for Windows, with right-click options from File Explorer and a viewer for protected files. For more information, see the [blog post announcement](https://aka.ms/aip-december-release) and preliminary documentation: [Azure Information Protection user guide](client-user-guide.md).

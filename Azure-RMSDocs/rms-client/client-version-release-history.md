...
The current preview version is **1.27.44.0** and has the following changes since the current GA version of the client.  
...
- For the Azure Information Protection scanner:
    ...
    - By default, the scanner now runs with a low integrity level for higher security in case you run the scanner with an account that has privileged rights. When the service account that runs the scanner has only the rights documented in the [scanner prerequisites](../deploy-use/deploy-aip-scanner.md#prerequisites-for-the-azure-information-protection-scanner), the low integrity level is not necessary and is not recommended because it negatively affects performance. You can use an advanced client setting to change the low integrity level for the scanner. [More information](../rms-client/client-admin-guide-customizations.md#run-the-scanner-with-a-medium-integrity-level). 

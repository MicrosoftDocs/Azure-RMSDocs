...
> [!TIP]
> If needed, apply additional security to specific documents using an additional on-premises key. For more information, see [Hold your own key (HYOK) protection](configure-adrms-restrictions.md) (classic client) or [Double Key Encryption (DKE) protection](plan-implement-tenant-key.md#double-key-encryption-dke-aip-unified-labeling-client-only).
> 

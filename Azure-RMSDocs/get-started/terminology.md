...
|Office Message Encryption|Frequently abbreviated to *OME*.<br /><br />The new Office 365 Message Encryption (OMEv2) capabilities provide the same email protection for internal and external users with automatic refresh of templates, and support for the bring your own key (BYOK) scenario. <br /><br /The previous OME implementation (OMEv1) was designed for external recipients only, required a mail flow rule for implementation, and did not support BYOK.|

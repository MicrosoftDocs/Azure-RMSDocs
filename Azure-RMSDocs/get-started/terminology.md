...
|Office Message Encryption|Frequently abbreviated to *OME*.<br /><br />The new Office 365 Message Encryption capabilities provide the same email protection for internal and external users with automatic refresh of templates, and support for the bring your own key (BYOK) scenario. Protection is built-in with native support for the Azure Rights Management service. <br /><br /The previous OME implementation was designed for external recipients only, required a mail flow rule, and did not support BYOK.|

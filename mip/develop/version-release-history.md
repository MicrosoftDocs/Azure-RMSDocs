...
- Added preview support for offline publishing in the Protection SDK.
  - Offline protection can be enabled by setting `ProfileProfileSettings.OfflinePublishing` to `true` in both C++ and .NET.
...
MIP SDK 1.13 introduces support for consuming files and emails protected with AES CBC mode generated by Word, Excel, PowerPoint, Outlook, Exchange Online, SharePoint Online, and MIP SDK-enabled applications opted in to CBC publishing. If your application uses the File SDK to consume any of these formats, it's important that you update the application to MIP SDK 1.13. In the second half of 2023, M365 Apps, Exchange Online, and SharePoint Online will begin to protect with CBC mode by default. Applications that have not updated will fail to properly decrypt files and emails.

MIP SDK 1.13 will continue to publish Office documents and emails in electronic codebook (ECB) mode. MIP SDK 1.14, scheduled for release in June of 2023, will enable CBC publishing by default. If you'd like to test CBC publishing ahead of time, enable the `UseCBCForOfficeFileEncryption` feature flag via `MipConfiguration.FeatureSettingsOverride()`.
